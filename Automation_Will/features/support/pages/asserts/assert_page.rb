class Assert_Page
 ...
 include Capybara::DSL
  ...
  def valida_url(url)
    if page.driver.current_url.include? url
      p "Página correta #{page.driver.current_url}"
 ...
 def valida_finalizacao_compra_debito_online(seguro)
  ...
  def valida_botao_retira_lista_lojista
    ...
    def valida_botao_proximo_desabilitado
      sleep(6)
      if find('#wizard-next-local')[:disabled]
  ...
  def valida_ambientes_mvp
    sleep(6)
    if find("Casa completa, Cozinha").visible?
      p "Ambientes selecionados"
    else
      raise "Ambientes não selecionados"
    end
  end
...
#Data: 16/02/2017
  def valida_exibicao_mensagem(elemento, mensagem)
    assert_selector(EL[elemento], text: MSG[mensagem], wait:25, visible:true)
  end
  ...
  def valida_caractere_especial(texto)
    case texto
    when "texto_NomeTitular"
      if texto.scan(/[a-zA-Z]/)
  ...
  def valida_inexistencia_texto(elemento,texto)
    if assert_no_selector(EL[elemento],text:texto)
      p "texto:#{texto} não encontrado na tela."
    else
      raise "texto:#{texto} encontrado. ERRO"
    end
  end

  def valida_existencia_texto(elemento,texto)
    if assert_selector(EL[elemento],text:texto)
      p "texto:#{texto} encontrado na tela."
    else
      raise "texto:#{texto} não encontrado. ERRO"
    end
  end

  def validar_limite_caracteres (elemento,limite)
  texto = find(EL[elemento]).lenght
     raise ('A quantidade de caracteres ultrapassou o limite permitido.') if texto > limite.to_i
  end


  def validar_limite_caracteres2
  texto = find('#wizard-details-textatea-description').value
  quantidade_caracteres = texto.size
  p 'QUANTIDADE DE CARACTERES NO CAMPO'
  p quantidade_caracteres
  p 'TIPO DE VARIAVEL'
  p quantidade_caracteres.class
  raise ('A quantidade de caracteres ultrapassou o limite permitido.') unless quantidade_caracteres > 450
  end

  def valida_pedido_enviado_mvp
    assert_selector('.wizard-success__order')
  end
